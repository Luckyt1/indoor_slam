...
int k = 0;
int idx = -1;
point_lio::esikf::NarrowESIKF<state_input, 24, input_ikfom> kf_input;
point_lio::esikf::NarrowESIKF<state_output, 30, input_ikfom> kf_output;
...
void pointBodyToWorld(const State & state, PointType const * const input, PointType * const output)
{
  const V3D point_lidar(input->x, input->y, input->z);
  const V3D point_world = point_lio::transformPointToWorld(
    state, point_lidar, extrinsic_est_en, Lidar_R_wrt_IMU, Lidar_T_wrt_IMU);
...
Eigen::Matrix<double, 24, 24> df_dx_input(state_input & s, const input_ikfom & in)
{
  ...
  vect3 acc_;
  acc_ = in.acc - s.ba;
  cov.template block<3, 3>(12, 3) = -s.rot.matrix() * skew_sym_mat(acc_);
...
Eigen::Matrix<double, 30, 30> df_dx_output(state_output & s, const input_ikfom &)
{
  Eigen::Matrix<double, 30, 30> cov = Eigen::Matrix<double, 30, 30>::Zero();
  cov.template block<3, 3>(0, 12) = Eigen::Matrix3d::Identity();
  cov.template block<3, 3>(12, 3) = -s.rot.matrix() * skew_sym_mat(s.acc);
  cov.template block<3, 3>(12, 18) = s.rot.matrix();
  cov.template block<3, 3>(12, 21) = Eigen::Matrix3d::Identity();
  cov.template block<3, 3>(3, 15) = Eigen::Matrix3d::Identity();
  return cov;
...
void h_model_input(
  ...
  point_lio::esikf::MeasurementData<double> & ekfom_data)
{
  ...
  for (int j = 0; j < time_seq[k]; j++) {
    ...
    pointBodyToWorld(s, &point_body_j, &point_world_j);
...
void h_model_IMU_output(
  state_output & s, point_lio::esikf::MeasurementData<double> & ekfom_data)
{
...
{
  if (use_imu_as_input) {
    pointBodyToWorld(kf_input.x_, pi, po);
  } else {
    pointBodyToWorld(kf_output.x_, pi, po);
  }
}
