...
std::string world_frame = "map", body_frame = "odom";
...
void readParameters(std::shared_ptr<rclcpp::Node> & nh)
{
  ...
  try {
    ...
    nh->declare_parameter<std::string>("publish.world_frame", "map");
    nh->get_parameter("publish.world_frame", world_frame);

    nh->declare_parameter<std::string>("publish.body_frame", "odom");
    ...
    nh->declare_parameter<bool>("pcd_save.filter_en", true);
    nh->get_parameter("pcd_save.filter_en", pcd_save_filter_en);
